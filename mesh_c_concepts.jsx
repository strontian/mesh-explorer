...
function BodyMap({ data, initialSelection }) {
  ...
  useEffect(() => {
    ...
    if (!treeNum?.startsWith("C") || !treeIndex.has(treeNum)) return;
    const rootTreeNum = treeNum.split(".")[0];
    const region = REGIONS.find(r => r.id === rootTreeNum);
    const special = SPECIAL_CFG.find(s => s.treeNum === rootTreeNum);
    const systemic = SYSTEMIC_CFG.find(s => s.treeNum === rootTreeNum);
    const nextSel = region
      ? { type:"region", id:rootTreeNum }
      : special
        ? { type:"special", id:rootTreeNum }
        : systemic
          ? { type:"systemicBranch", id:rootTreeNum }
          : { type:"region", id:rootTreeNum };
  ...
  function selectDiagramRegion(event, id) {
    ...
    selectNavigation({ type:"region", id });
  }
  ...
  return (
    <div style={{ background:BG, height:"100%", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      <MeshPageHeader
        letter="C"
        title="Diseases"
        description="Disease branches mix body systems, systemic causes, injuries, infections, animal disease, chemically induced disease, and occupational conditions."
        color={TREE_COLOR}
      />
          ...
          <div style={{ border:"1px solid #ffffff14", borderRadius:8, background:"linear-gradient(180deg,#ffffff05,transparent)", padding:"12px 8px 10px" }}>
            <svg viewBox="0 0 523 740" onClick={() => selectNavigation({type:"overview"})} style={{ display:"block", width:"100%", maxHeight:590, margin:"0 auto", cursor:"default" }} aria-label="Disease body region selector">
              ...
              <path
                ...
                onClick={(event) => selectDiagramRegion(event, "C17")}
                onMouseEnter={() => setHovReg("C17")}
                onMouseLeave={() => setHovReg(null)}
              />

              <ellipse onClick={(event) => selectDiagramRegion(event, "C10")} onMouseEnter={() => setHovReg("C10")} onMouseLeave={() => setHovReg(null)} cx={249} cy={135} rx={34} ry={42} fill={rFill("C10","#9B72CF")} stroke={rStroke("C10","#9B72CF")} strokeWidth={rSW("C10")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <ellipse onClick={(event) => selectDiagramRegion(event, "C11")} onMouseEnter={() => setHovReg("C11")} onMouseLeave={() => setHovReg(null)} cx={238} cy={130} rx={11} ry={9} fill={rFill("C11","#FF9A9E")} stroke={rStroke("C11","#FF9A9E")} strokeWidth={rSW("C11")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <ellipse onClick={(event) => selectDiagramRegion(event, "C11")} onMouseEnter={() => setHovReg("C11")} onMouseLeave={() => setHovReg(null)} cx={260} cy={130} rx={11} ry={9} fill={rFill("C11","#FF9A9E")} stroke={rStroke("C11","#FF9A9E")} strokeWidth={rSW("C11")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <path onClick={(event) => selectDiagramRegion(event, "C09")} onMouseEnter={() => setHovReg("C09")} onMouseLeave={() => setHovReg(null)} d="M224,139 C214,143 213,158 225,161 L230,154 C226,150 226,145 229,142 Z M274,139 C284,143 285,158 273,161 L268,154 C272,150 272,145 269,142 Z" fill={rFill("C09","#F0B8C8")} stroke={rStroke("C09","#F0B8C8")} strokeWidth={rSW("C09")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <rect onClick={(event) => selectDiagramRegion(event, "C07")} onMouseEnter={() => setHovReg("C07")} onMouseLeave={() => setHovReg(null)} x={228} y={178} width={42} height={34} rx={12} fill={rFill("C07","#F0B8C8")} stroke={rStroke("C07","#F0B8C8")} strokeWidth={rSW("C07")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <rect onClick={(event) => selectDiagramRegion(event, "C08")} onMouseEnter={() => setHovReg("C08")} onMouseLeave={() => setHovReg(null)} x={180} y={220} width={66} height={78} rx={18} fill={rFill("C08","#4ECDC4")} stroke={rStroke("C08","#4ECDC4")} strokeWidth={rSW("C08")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <rect onClick={(event) => selectDiagramRegion(event, "C14")} onMouseEnter={() => setHovReg("C14")} onMouseLeave={() => setHovReg(null)} x={247} y={220} width={44} height={78} rx={16} fill={rFill("C14","#E07A5F")} stroke={rStroke("C14","#E07A5F")} strokeWidth={rSW("C14")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <rect onClick={(event) => selectDiagramRegion(event, "C15")} onMouseEnter={() => setHovReg("C15")} onMouseLeave={() => setHovReg(null)} x={292} y={220} width={26} height={78} rx={12} fill={rFill("C15","#F4A261")} stroke={rStroke("C15","#F4A261")} strokeWidth={rSW("C15")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <rect onClick={(event) => selectDiagramRegion(event, "C06")} onMouseEnter={() => setHovReg("C06")} onMouseLeave={() => setHovReg(null)} x={172} y={300} width={78} height={74} rx={18} fill={rFill("C06","#81B29A")} stroke={rStroke("C06","#81B29A")} strokeWidth={rSW("C06")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <rect onClick={(event) => selectDiagramRegion(event, "C18")} onMouseEnter={() => setHovReg("C18")} onMouseLeave={() => setHovReg(null)} x={251} y={300} width={37} height={74} rx={14} fill={rFill("C18","#A8DADC")} stroke={rStroke("C18","#A8DADC")} strokeWidth={rSW("C18")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <rect onClick={(event) => selectDiagramRegion(event, "C19")} onMouseEnter={() => setHovReg("C19")} onMouseLeave={() => setHovReg(null)} x={289} y={300} width={37} height={36} rx={12} fill={rFill("C19","#D8A8D8")} stroke={rStroke("C19","#D8A8D8")} strokeWidth={rSW("C19")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <rect onClick={(event) => selectDiagramRegion(event, "C20")} onMouseEnter={() => setHovReg("C20")} onMouseLeave={() => setHovReg(null)} x={289} y={338} width={37} height={36} rx={12} fill={rFill("C20","#A8D8C8")} stroke={rStroke("C20","#A8D8C8")} strokeWidth={rSW("C20")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              <rect onClick={(event) => selectDiagramRegion(event, "C12")} onMouseEnter={() => setHovReg("C12")} onMouseLeave={() => setHovReg(null)} x={176} y={376} width={146} height={62} rx={24} fill={rFill("C12","#A8DADC")} stroke={rStroke("C12","#A8DADC")} strokeWidth={rSW("C12")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              {[
                ...
                <path key={i} onClick={(event) => selectDiagramRegion(event, "C05")} onMouseEnter={() => setHovReg("C05")} onMouseLeave={() => setHovReg(null)} d={d} fill={rFill("C05","#DDB892")} stroke={rStroke("C05","#DDB892")} strokeWidth={rSW("C05")} style={{ cursor:"pointer", transition:"all 0.15s" }} />
              ))}

              {diagramLabels.map(([id, color, x1, y1, x2, y2, tx, ty, label]) => (
                <g
                  key={id}
                  onClick={(event) => selectDiagramRegion(event, id)}
...
export default function MeshCConcepts({ initialSelection } = {}) {
  const { data, loading } = useCData();

  return (
    <div style={{ width: "100%", height: "100%", background: BG }}>
      <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;600;700&display=swap" rel="stylesheet" />
      {loading ? <Loading /> : <BodyMap data={data} initialSelection={initialSelection} />}
    </div>
  );
}
